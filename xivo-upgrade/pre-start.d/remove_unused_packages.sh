...
renamed_packages="xivo-agid xivo-base-config xivo-fetchfw xivo-web-interface-config xivo-provd xivo-dxtora xivo-dhcpd-update xivo-monitoring xivo-web-interface xivo-sysconfd"
postrm_webi_config="/var/lib/dpkg/info/xivo-web-interface-config.postrm"

echo "cleanup outdated config files"
if [ -f $postrm_webi_config ]; then
    rsync -av /etc/xivo/web-interface /tmp/ > /dev/null
    rm -rf $postrm_webi_config > /dev/null
fi

# cleanup xivo-base-config.postrm file to allow package purge
base_config_postrm="/var/lib/dpkg/info/xivo-base-config.postrm"
...
# cleanup xivo-sysconfd backup directory
old_sysconfd_directory="/var/backups/xivo-sysconfd"
...
# sync old xivo-provd data
old_provd_directory="/var/lib/xivo-provd"
...
old_web_interface_log_directory="/var/log/xivo-web-interface"
old_web_interface_config_directory="/etc/xivo/web-interface"
...
if [ -d /tmp/web-interface ]; then
    rsync -av /tmp/web-interface/ /etc/xivo/web-interface/ > /dev/null
